...
public class NiveauGraphique extends JComponent implements Observateur {
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < (lignes); i++) {
			h = Math.round((float) i * height);
			for (int j = 0; j < (colonnes); j++) {
				if (jeu.valeur(i, j) == 0) continue;
				if (i % 2 == 1)
					l = j * largeurCase + largeurCase / 2;
				else
					l = j * largeurCase;
				g.drawImage(assetsPlateau[jeu.valeur(i, j)], l, h,
						largeurCase, hauteurCase, this);
			}
		}

		if (jeu.etatCourant() == Etats.Initialisation) {
			for (int[] pingouins : jeu.getPingouins(1)) {
				x = pingouins[0];
				y = pingouins[1];
		...
		if (jeu.etatCourant() != Etats.Initialisation) {
			for (int[] pingouins : jeu.getPingouins(jeu.joueurCourant + 4)) {
				x = pingouins[0];
				y = pingouins[1];
				h = Math.round((float) x * height);
				if (x % 2 == 1) {
